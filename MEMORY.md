...
- **IP:** 213.171.25.85
- **Пользователь:** user1 (passphrase-ключ)
- **Хостнейм:** vm-low4-8
- **Ключ:** `~/.ssh/new-vps-key`, парольная фраза `Zz1893645`
- **Подключение:** `ssh -i ~/.ssh/new-vps-key user1@213.171.25.85`
- **ОС:** Ubuntu 24.04.4 LTS (финал, апгрейд НЕ делаем)
- **Статус:** чистый VPS, настройка не начата
- **25.07.2026:** Попытка апгрейда до 26.04 LTS сломала сервер → переустановка через Cloud.ru. Решение: остаёмся на 24.04 LTS.
...
- **Проблема:** DeepSeek V4 Flash «залипает» на утренних запусках (08:00–09:00 МСК = 13:00–14:00 CST, китайский вечерний прайм-таим). Модель принимает запрос, но не возвращает ответ за 300+ сек.
- **Симптом:** `model-call-started` timeout ровно на 300 сек
- **Решение:** Для задач с большими промптами (статистика жалоб, Катя) — primary **deepseek-chat**, fallback **deepseek-v4-flash** + таймаут 900 сек
- **deepseek-chat** стабильнее: отвечает быстрее, не «залипает», хотя дороже ($0.27/$1.1 против $0.14/$0.28 за млн токенов)
- **CHECK**: для stats-inheritance и Кати (осн.) глубокие промпты ~30-50k токенов — deepseek-chat отработает за 150-200 сек, что влезает в таймаут
...
9. **Анализ ≠ действие.** Если Кирилл просит «проанализируй»/«разбери»/«почему» — только анализ и вывод. Изменения (cron-промпты, конфигурация, системные файлы) — только после явного «примени», «сделай», «исправь». Двухшаговый процесс: анализ → вопрос «Применить?» → ждать ответа → действие.
...
### Правило оформления cron-отчётов (17.07.2026, расширено 23:45)
- **На КАЖДОЙ странице cron-отчёта ОБЯЗАТЕЛЬНО указывать:**
  1. Дату и время выполнения задачи
  2. Дату и время следующего запуска по расписанию
  3. Модель LLM, использованную для генерации
  4. Сколько времени ушло на выполнение задачи
  5. Отметку verify-агента (если применимо)
- **Отчёты:** report-roza.html, report-irina.html, report-lena.html, report-danil.html, stats-inheritance.html и любые новые
- Формат в блоке `<div class="meta">`:
  `📅 Запуск: ДД.ММ.ГГГГ ЧЧ:ММ · Следующий: ДД.ММ.ГГГГ (день) ЧЧ:ММ`
  `🧠 Модель: название · ⏱ Время: Ns · 🔍 Verify: agent-name (model, timeout)`
- Правило бессрочное, для ВСЕХ cron-генерируемых страниц

## 🧑‍⚖️ Адвокат — личный агент Кирилла (создан 12.07.2026)

### Концепция
- Персональный юридический + психологический агент для защиты прав отца
- Кирилл — клиент, дочь Василиса (12.04.2020)
- Вызов: «Адвокат» или «@адвокат» в начале сообщения в этом чате
- Маршрутизация: Лунт → sessions_send(agentId="kirill-family-advocate") → ответ
- Абсолютная конфиденциальность — детали не уходят в отчёты/cron

### Три компетенции
1. **Семейный юрист** — СК РФ (57, 61, 63, 65, 66, 67, 69, 80, 81), алименты (независимость от общения), права отца, органы опеки
2. **Эксперт по абьюзу** — PAS (синдром отчуждения родителя), газлайтинг, обесценивание
3. **Переговорщик** — Grey Rock, Broken Record, эскалационная лестница, правила ответа на провокации/угрозы/ложь

### База знаний
- `knowledge/kirill/family-law.md` — статьи СК РФ + психология абьюза
- Верификация: web_fetch zakonrf.info/sk/ для проверки статей

### Skill
- `kirill-family-advocate` (SKILL.md через workshop, pending)

### Формат ответа
```
🔍 Анализ: [манипуляция/абьюз/нарушение]
🧠 Психология: [паттерн]
📋 Закон: [ст. СК РФ]
✏️ Ответ (копируй): [...]
📌 Действие: [...]
...
### Запреты (жёстко)
- Не оскорблять мать
- Не советовать конфликт при ребёнке
- Не имитировать её ответы
- Не давать гарантий
- Ответ готов к копированию и отправке

### Правила маршрутизации
- В AGENTS.md прописана маршрутизация по триггеру «Адвокат»
- Только Кирилл (346428630) может вызывать Адвоката
...
### «Новости наследства» — lena-chat-agent (создан 18.07.2026, оптимизирован 18.07.2026)
...
- **Оптимизация 18.07.2026:** personality (энергичный редактор, эмодзи), 4 команды с шаблонами, поиск по теме, 3 fallback-сценария

### РЖД 1Р-37R — пайплайн (создан 18.07.2026)
- **3 cron-задачи:** rzd-search-agent (830601f5, 23:50 будни, MOEX ISS API → rzd-raw.json), rzd-verify-agent (061e128b, 23:55 будни, валидация + аналитика >0.5% → rzd-verified.json), rzd-html-agent (0da4a1e1, 23:58 будни, HTML + публикация → Лена)
- **Модель:** deepseek-chat (весь пайплайн), доставка announce → telegram:254785028
- **HTML-шаблон:** templates/report-rzd-template.html
- **Первый запуск:** понедельник 20.07.2026 23:50 МСК
...
- Получает: report-lena.html ежедневно 02:20 МСК + report-rzd.html будни 23:58 МСК
- **Агент:** «Новости наследства» (lena-chat-agent) — прямой чат через bindings
- Дедупликация новостей: `agents/shared/lena-news-seen.md`
- **Skill:** `lena-news-expert` (applied 28.06, обновлён 18.07) — 10 категорий, 14 доменов, лимиты ≤15/search, ≤10/verify, ≤7/html
...
| 3a | 🔍 lena-search-agent: поиск новостей | ежедневно 02:00 | deepseek-v4-flash | 87d3aaef → lena-raw.json (≤15) |
| 3b | 🔍 lena-verify-agent: верификация | ежедневно 02:10 | deepseek-v4-flash | ad4d79bb → lena-verified.json (≤10) |
| 3c | 📋 lena-html-agent: HTML + публикация | ежедневно 02:20 | deepseek-v4-flash | 43ce4092 → Лена + сайт (≤7) |
| ⚡ | **Все 3 — под lena-chat-agent** (оптимизированы 18.07) |
| 4a | 🚂 rzd-search-agent: сбор MOEX ISS | будни 23:50 | deepseek-chat 120s | 830601f5 → rzd-raw.json |
| 4b | 🚂 rzd-verify-agent: валидация + аналитика | будни 23:55 | deepseek-chat 180s | 061e128b → rzd-verified.json |
| 4c | 🚂 rzd-html-agent: HTML РЖД + публикация | будни 23:58 | deepseek-chat 120s | 0da4a1e1 → Лена + сайт |
| ⚡ | **РЖД-пайплайн создан 18.07, первый запуск пн 20.07** |
...
### 30.06.2026 — Катя: расписание и модели
- DeepSeek API иногда «залипает» на первом утреннем запросе (stall на 2+ мин), Gateway абортит соединение. При повторном запуске всё ок.
- Основной запуск перенесён на 08:30 (позже индексация SearXNG)
- Добавлен резервный запуск (08:45) на случай сбоя
- Модель: deepseek-v4-flash (primary) → deepseek-chat → glm-5.1 (fallback, 600s timeout)

### 03.07.2026 — Катя: смена приоритета моделей
- DeepSeek V4 Flash стал стабильно «залипать» на утренних запусках (08:00–09:00 МСК)
- API-ключ валидный, но модель зависает на 300+ сек и Gateway прерывает соединение
- **Решение:** основная модель для утренних задач Кати (сводка жалоб, резерв, stats-inheritance) — **deepseek-chat**, fallback — deepseek-v4-flash
- deepseek-chat чуть дороже, но стабильно отрабатывает за 60–90 сек
...
### 30.06.2026 — ФармТендеролог: завершён редизайн, отказ от LegalMCP
- Создана страница продукта: pharm-tenderolog-product.html
- Заголовок «ФармТендеролог» на zakupki.html — стиль градиент как на странице продукта
- tagline на pharm-tenderolog-product.html заменён на «Ваш цифровой консультант по 44-ФЗ / 223-ФЗ (лекарственные препараты и медизделия)»
- Слово «Катрин» удалено со всех публичных страниц
- **LegalMCP отключён** полностью из-за исчерпания лимита (84/100) и устаревшей базы
- **Верификация через consultant.ru** (web_fetch) — работает, показал реальные редакции 2025 года
- **224-ФЗ заменён на 223-ФЗ** (правильный закон для закупок госкомпаний)
- Создан health-check: ежедневно 09:00, проверяет валидность zakupki-result.json (позже заменён на health-check zakupki-purchases в 07:20)
- Создан скрипт legalmcp-update-v3.py — принимает данные из аргументов, публикует через git
- Оценка качества продукта после миграции: 70%

### 30.06.2026 — ФармТендеролог: живые закупки с ЕИС (03:04)
- Создан `scripts/parse-zakupki.py` — парсинг zakupki.gov.ru по 5 ключевым запросам (лекарственные средства, медицинские изделия, лекарственные препараты, медизделия, фармацевтическая продукция)
- Схлопывание дублей по номеру закупки, сортировка по дате, лимит 100 записей
- Результат: `zakupki-purchases.json` — публикуется на GitHub Pages
- Страница `zakupki.html` — новый блок с таблицей закупок (закон, номер, статус, заказчик, объект, цена, дата), данные через fetch с клиента
- Страница продукта `pharm-tenderolog-product.html` — обновлены источники: LegalMCP → ЕИС + consultant.ru
- План отката: `rollback-pharm-tenderolog.md`
- Создан `rollback-pharm-tenderolog.md` с 4 шагами отката

**Cron-задачи по Катрин (будни, до 08:00):**
| Время | Задача |
|-------|--------|
| 🕖 07:00 | 📋 Парсинг закупок с zakupki.gov.ru |
...
## 🤝 katrin-agent → auditor-agent: связка верификации (02.07.2026)

### Архитектура
- **katrin-agent** — фронтальный агент Катрин. Отвечает в Telegram, генерирует документы, консультирует
- **auditor-agent** — асинхронный верификатор. Работает только через sessions_send от katrin-agent

### Протокол связи
1. Катрин задаёт вопрос → katrin-agent готовит ответ с цитатами НПА
2. Перед выдачей — katrin-agent отправляет **все юридические факты** (статус нормы, даты, ссылки) в auditor-agent через sessions_send
3. auditor-agent верифицирует через consultant.ru → pravo.gov.ru → профильные ресурсы
4. auditor-agent возвращает вердикт в формате:
   ```
   🔍 auditor-agent:
   ...
   • Статус: ✅ / ❌ / 🔄 / ❓
   ```
5. katrin-agent выдаёт ответ + включает в него вердикт аудитора

### Документация
- **katrin-agent:** AGENTS.md п. 5 — обязательная верификация
- **auditor-agent:** AGENTS.md п. 2 — верификация фактов, п. 5 — формат вердикта
- **auditor-agent:** TOOLS.md — приоритет источников верификации
