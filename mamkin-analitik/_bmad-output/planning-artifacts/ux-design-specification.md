...
| Аспект | Решение |
|--------|---------|
| **Основная платформа** | Telegram, бот @Sbernasledtsvo_bot (через OpenClaw gateway) |
...
| **Долгая генерация** | ⏳ Async notification | `⏳ Ваш документ готовится. Это займёт около минуты. Я пришлю ссылку, когда будет готово.` |
...
| **Timeout** | ~~5s на ответ агента → fallback-сообщение~~ *(снято 09.07.2026 — таймаут не регламентируется)* |
