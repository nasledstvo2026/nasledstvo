...
## 🧑‍⚖️ Адвокат — Личный агент Кирилла

### Триггер вызова
- Сообщения, начинающиеся с «Адвокат» или «@адвокат» в этом чате
- Лунт распознаёт триггер, маршрутизирует агенту `kirill-family-advocate` через sessions_send
- Агент отвечает → Лунт возвращает ответ Кириллу

### Правило маршрутизации
Если сообщение Кирилла (346428630) начинается с «Адвокат» или «@адвокат»:
1. Взять полный текст сообщения Кирилла (всё что после «Адвокат, »)
2. Прочитать актуальный портрет: `knowledge/kirill/mother-portrait.md`
3. Сформировать запрос агенту в формате:
   ```
   Прочитай портрет: knowledge/kirill/mother-portrait.md
   
   Адвокат, запрос: [текст Кирилла]
   
   Новый кейс. Сверь с портретом, учти накопленные данные.
   ```
   Адвокат, запрос: [текст Кирилла]
   
   Ответь строго 5 строками, без ничего лишнего:
   🔍 Анализ: 
   🧠 Психология: 
   📋 Закон: (используй ТОЛЬКО ст. 57,61,63,65,66,67,69,80,81 СК РФ, не выдумывай других)
   ✏️ Ответ (копируй): (пиши от лица Кирилла, НЕ от лица ребёнка, сухо, без оскорблений, Grey Rock)
   📌 Действие:
   ```
3. Отправить запрос через sessions_send(agentId="kirill-family-advocate", message=...)
4. Дождаться ответа
5. Вернуть ответ Кириллу как есть (формат уже правильный)
6. Не фиксировать детали в основной память — конфиденциально

### Режимы работы
1. **«Адвокат, [ситуация]»** — анализ ситуации, закон, готовый ответ
2. **«Адвокат, [текст её сообщения]»** — анализ её сообщения + контр-ответ
3. **«Адвокат, как [действие]»** — инструкция (соглашение, иск, жалоба)
