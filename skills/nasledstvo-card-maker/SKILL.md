...
## ⚠️ ГЛАВНОЕ ПРАВИЛО: сначала посмотри, что уже есть

**НЕ переписывай существующие страницы под шаблоны этого скилла.** Сначала открой целевой файл и посмотри его реальную разметку. Добавляй новое в **том же формате**, что уже используется на странице. Скилл описывает эталонный glass-стиль для НОВЫХ страниц; существующие страницы могут иметь другой (исторический) формат — его надо сохранять.

### index.html (главная) — ОСОБЫЙ СЛУЧАЙ

Актуальная главная использует **aura-плашки** (текстовые, без картинок), НЕ glass-шаблоны ниже и НЕ анимированные пузырьки:

```html
<a href="ССЫЛКА.html" class="card-link">
  <div class="card ЦВЕТ aura">
    <div class="card-header"><div class="card-header-row"><h2>ЗАГОЛОВОК</h2></div></div>
    <div class="desc">ОПИСАНИЕ (1-2 предложения)</div>
  </div>
</a>
```

- Цвета: `red` / `orange` / `cyan` / `purple` / `pink` / `green` (из цветовой карты ниже).
- Фон главной: `bg-grid` + `bg-glow` + `particles` (aidj-стиль) — **НЕ добавлять** `.bubbles`.
- Секция «Феникс» внизу главной — блок `.section` со ссылками `.item`, не трогать и не дублировать.
- Пример реальной плашки на главной (04.08.2026):
```html
<!-- Феникс -->
<a href="https://nasledstvo2026.github.io/Phoenix/index.html" class="card-link">
  <div class="card purple aura">
    <div class="card-header"><div class="card-header-row"><h2>Феникс</h2></div></div>
    <div class="desc">AI-консультант по 425-ФЗ: налоговое взыскание, ПОС, ПНО, розыск счетов.</div>
  </div>
</a>
```
...
## Анимированные пузырьки — ТОЛЬКО для НОВЫХ страниц в glass-стиле

⚠️ **НЕ добавлять на существующую главную index.html** — там уже свой фон (bg-grid/bg-glow/particles). Пузырьки уместны только при создании новой страницы с нуля в glass-стиле.
...
## Шаблон цветной карточки (для НОВЫХ страниц / glass-стиля)
...
3. **Пузырьки — только для новых glass-страниц.** На существующую главную index.html НЕ добавлять (там bg-grid/bg-glow/particles).
...
13. **Сначала посмотри, потом меняй.** Открой целевой файл, определи его реальный формат и добавляй в нём. Не заменяй существующую разметку шаблонами скилла.

## Проверка перед публикацией

- [ ] Целевой файл открыт, формат определён (aura / glass / другой)
- [ ] Новое добавлено в существующем формате страницы, ничего не переписано
- [ ] На главную НЕ добавлены `.bubbles` (там particles-фон)
